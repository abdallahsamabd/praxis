...
pub(super) fn is_retriable(policy: &RetryPolicy, outcome: RetryOutcome) -> bool {
    match outcome {
        ...
        RetryOutcome::RefusedStream => policy
            .retriable_conditions
            .contains(&RetriableCondition::RefusedStream),
        RetryOutcome::StatusCode(code) => {
            let in_list = policy.retriable_status_codes.iter().any(|c| c.get() == code);
            let is_5xx = (500..600).contains(&code)
                && policy
                    .retriable_conditions
                    .contains(&RetriableCondition::Status5xx);
            in_list || is_5xx
        },
    }
}
...
pub(super) fn classify_error(e: &pingora_core::Error) -> RetryOutcome {
    ...
    match e.etype() {
        ...
        ErrorType::H2Error | ErrorType::H2Downgrade | ErrorType::InvalidH2 => {
            RetryOutcome::RefusedStream
        },
        ErrorType::HTTPStatus(code) => RetryOutcome::StatusCode(*code),
        _ => {
            // Treat other upstream errors as connect-like for policy matching.
    ...
    if let Some(timeout_ms) = policy.request_timeout_ms {
        ...
        if elapsed >= Duration::from_millis(timeout_ms) {
            debug!(
                ...
                timeout_ms,
                "skipping retry: overall request timeout exceeded"
            );
            return RetryDecision::DoNotRetry;
        }
    }
...
mod tests {
    use praxis_core::config::{HttpStatusCode, RetriableCondition, RetryPolicy};
