...
impl RoundRobin {
    ...
    pub(crate) fn select(&self, health: Option<&ClusterHealthState>, exclude: &[Arc<str>]) -> Option<Arc<str>> {
        ...
        select_by_weight_excluding(&self.endpoints, tick, self.total_weight, exclude)
    }
    ...
    fn select_healthy(
        ...
        let candidates: Vec<&WeightedEndpoint> = self
            .endpoints
            .iter()
            .filter(|ep| {
            ...
            .collect();
        let healthy_weight: usize = candidates.iter().map(|ep| ep.weight as usize).sum();
        if healthy_weight == 0 {
            return None;
        }
        Some(select_by_weight(
            &candidates.iter().map(|ep| (*ep).clone()).collect::<Vec<_>>(),
            tick,
            healthy_weight,
        ))
    }
...
fn select_by_weight_excluding(
    endpoints: &[WeightedEndpoint],
    tick: usize,
    _total_weight: usize,
    exclude: &[Arc<str>],
) -> Option<Arc<str>> {
    let candidates: Vec<&WeightedEndpoint> = endpoints
        .iter()
        .filter(|ep| !is_excluded(&ep.address, exclude))
        .collect();
    let weight: usize = candidates.iter().map(|ep| ep.weight as usize).sum();
    if weight == 0 {
        return None;
    }
    let owned: Vec<WeightedEndpoint> = candidates.iter().map(|ep| (*ep).clone()).collect();
    Some(select_by_weight(&owned, tick, weight))
}
