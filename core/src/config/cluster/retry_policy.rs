...
impl TryFrom<u16> for HttpStatusCode {
    ...
    fn try_from(value: u16) -> Result<Self, Self::Error> {
        if (100..=599).contains(&value) {
            ...
            Err(format!(
                "http status code must be in 100..=599, got {value}"
            ))
        }
    }
}
...
impl TryFrom<f64> for BudgetPercent {
    ...
    fn try_from(value: f64) -> Result<Self, Self::Error> {
        if !value.is_finite() {
            return Err(format!(
                "retry_budget.percent must be finite, got {value}"
            ));
        }
        if !(0.0..=100.0).contains(&value) {
            return Err(format!(
                "retry_budget.percent must be in 0.0..=100.0, got {value}"
            ));
        }
        Ok(Self(value))
    }
...
impl RetryPolicy {
    ...
    pub fn merge_override(&self, route: &Self) -> Self {
        Self {
            ...
            retry_body_limit_bytes: route
                .retry_body_limit_bytes
                .or(self.retry_body_limit_bytes),
            allow_non_idempotent: route.allow_non_idempotent || self.allow_non_idempotent,
        }
    }
}
...
"#;
        ...
        assert_eq!(policy.effective_max_retries(), 3);
        assert_eq!(policy.retriable_status_codes.len(), 3);
        assert_eq!(policy.retriable_status_codes[1].get(), 503);
...
"#;
    ...
    fn legacy_default_is_connect_failure_only() {
        ...
        assert_eq!(policy.effective_max_retries(), 3);
        assert_eq!(
            policy.retriable_conditions,
            vec![RetriableCondition::ConnectFailure]
        );
